...
(function() {
    ...
    const app = new Vue({
      ...
      methods: {
        ...
        getMoreData: function() {
          ...
          getDailyRankFromApi(this.page, this.rankType, this.rankDay).then(res => {
            if (res.length === 0) {
              this.noMore = true;
  ...
  function getDailyRankThumb(today) {
    return fetchData('https://mk-pixiv.kanata.ml/?r=api%2Fpixiv-json&_t=' + today);
  }
  function getDailyRankSource(today) {
    return fetchData('https://mk-pixiv.kanata.ml/?r=api%2Fsource-json&_t' + today);
  }

  async function getDailyRankFromApi(page, mode, date) {
    const _t = new Date().toLocaleString('ja-JP-u-ca-japanese').replace(/[\s/:]+/g, '').slice(0, -4);
    const res = await fetchData(`https://pixiv-api.kanata.ml/v2?type=rank&mode=${mode}&page=${page}&date=${date}&_t=${_t}&_vercel_no_cache=1`);
